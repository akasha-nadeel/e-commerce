...
No `tailwind.config.js`. Tokens and theme live in `src/app/globals.css` via `@import "tailwindcss"` + `@theme inline`. Brand palette: ink `#0c0c0d`, gold `#c79a4b`, bone `#f3f1ea`. Type stack leads with Apple's SF Pro (`-apple-system`) and falls back to **Inter** (loaded via `next/font`) — despite the README mentioning "Archivo," the code uses Inter. Path alias `@/*` → `./src/*`.
